...
export function ProductActionSheet({ product, action, onClose, onConfirm }: ProductActionSheetProps) {
  ...
  const getQuantityAdjective = () => {
    if (!product) return 'Quanti';
    const name = product.name.trim().toLowerCase();
    const lastChar = name.slice(-1);
    const num = Number(quantity) || 1;

    if (num === 1) {
      if (lastChar === 'a') return 'Quanta';
      return 'Quanto';
    } else {
      if (lastChar === 'a') return 'Quante';
      return 'Quanti';
    }
  };
          ...
          >
            ...
            <div className="p-6 pt-0 overflow-y-auto">
              <p className="text-stone-600 dark:text-stone-400 mb-6 text-lg">
                {getQuantityAdjective()} <span className="font-bold text-stone-900 dark:text-stone-100">{product.name}</span> hai {isConsume ? 'consumato' : 'buttato'}?
                <span className="block text-sm mt-1 opacity-70">(Disponibili: {product.quantity} {product.unit})</span>
              </p>
