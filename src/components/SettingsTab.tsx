...
import { Bell, Clock, CalendarDays, Moon, Sun, LayoutGrid, List, UtensilsCrossed, User, LogOut, Trash2, CheckCircle, ShieldAlert } from 'lucide-react';
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl p-5 shadow-sm border border-stone-100 dark:border-stone-800">
            ...
            <div className="mb-2">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Es. Noci, crostacei, fragole (separati da virgola)"
                  value={settings.intolerances || ''}
                  onChange={(e) => onUpdate({ intolerances: e.target.value })}
                  className="w-full px-4 py-3 pr-10 border border-stone-200 dark:border-stone-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-stone-50 dark:bg-stone-800 dark:text-white text-base"
                />
                {settings.intolerances && settings.intolerances.trim().length > 0 && (
              ...
              {settings.intolerances && settings.intolerances.trim().length > 0 && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-3 p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 rounded-xl"
                >
                  <p className="text-xs font-medium text-emerald-800 dark:text-emerald-400 mb-2 flex items-center gap-1.5">
                    <ShieldAlert className="w-3.5 h-3.5" />
                    Eviteremo ricette con:
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {settings.intolerances.split(',').map((item, idx) => {
                      const trimmed = item.trim();
                      if (!trimmed) return null;
                      return (
                        <span key={idx} className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-white dark:bg-stone-800 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700/50 shadow-sm capitalize">
                          {trimmed}
                        </span>
                      );
                    })}
              ...
              <p className="text-xs text-stone-500 dark:text-stone-400 mt-2">L'IA escluderà automaticamente questi ingredienti dalle tue ricette.</p>
            </div>
          </div>
